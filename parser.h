...
public:
  ...
  Term* createTerm(){
    ...
    if(token == VAR){
      //cout<<"fff"<<endl;
      map_iterator=map_variable.find(symtable[_scanner.tokenValue()].first);
      if(map_iterator==map_variable.end()){
        //cout<<"ccc"<<endl;
        Variable * x =new Variable(symtable[_scanner.tokenValue()].first);
        map_variable.insert(pair<string,Variable*>(symtable[_scanner.tokenValue()].first,x));
        return x;
      }else{
  ...
  string getResult(){
    string result;
    result= _expStack.top()->getResult()+".";
    return result;
  }
...
private:
  ...
  //MatchExp* _root;
  stack<Exp*> _expStack;
  map<string,Variable*> map_variable;
  map<string,Variable*>::iterator map_iterator;
};
